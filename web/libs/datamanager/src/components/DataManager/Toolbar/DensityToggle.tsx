...
export const DensityToggle = densityInjector(
  observer(({ size, onChange, storageKey, view, ...rest }: DensityToggleProps) => {
    ...
    return (
      ...
      >
        <Tooltip title={t("Comfortable density")}>
          <div>
            <RadioGroup.Button
              value={DENSITY_COMFORTABLE}
              aria-label={t("Comfortable density")}
        ...
        <Tooltip title={t("Compact density")}>
          <div>
            <RadioGroup.Button value={DENSITY_COMPACT} aria-label={t("Compact density")} data-testid="density-compact">
              <IconRows4 />
            </RadioGroup.Button>
          </div>
        </Tooltip>
